...
fun main(args : Array<String>)
{
    ...
    val comune_neamt = arrayOf<Comuna>  (Comuna("DumbravaRosie"))


    val orase_iasi = arrayOf <Oras> (Oras("Pascani",34200,210,2),Oras("Iasi",800000,3400,7))
    val orase_neamt = arrayOf <Oras> (Oras("PiatraNeamt",300000,1780,1),Oras("Roman",150000,630,1))
    ...
    /*println("\n\n-----------------------------------------------------------------")
    val capital_visitor= EsteCapitalaVisitor()  // nu stiu ce face functia .??
    ...
    toate_satele.forEach {
        println("${it.GetNume()} estecapitala:${it.accept(capital_visitor)}")
    }
    ...
    println("${capitala.GetNume()} estecapitala:${capitala.accept(capital_visitor)}")*/
}